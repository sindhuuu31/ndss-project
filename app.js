...
const trafficChart = new Chart(ctx, {
        ...
        datasets: [
            {
                ...
                data: [], borderColor: '#00f0ff', backgroundColor: 'rgba(0, 240, 255, 0.1)',
                borderWidth: 2, tension: 0.4, fill: true, yAxisID: 'y'
            },
            {
                label: 'Entropy',
                data: [], borderColor: '#ff3366', backgroundColor: 'transparent',
    ...
    options: {
        ...
        scales: {
            x: { grid: { color: 'rgba(100, 150, 255, 0.1)' } },
            y: { type: 'linear', display: true, position: 'left', grid: { color: 'rgba(100, 150, 255, 0.1)' } },
            y1: { type: 'linear', display: true, position: 'right', grid: { drawOnChartArea: false }, min: 0, max: 10 }
        }
    }
});
...
document.getElementById('manual-form').addEventListener('submit', async function(e) {
    ...
    modalStatus.style.color = 'var(--text-main)';
    modalStatus.innerText = 'CONNECTING TO AI ENGINE...';
    modalReason.innerHTML = '<div style="text-align:center; padding: 40px; font-size: 1.5rem; color: var(--cyan)">Processing Parameter Matrix through Random Forest...</div>';
    
    try {
        const response = await fetch('/api/manual', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ packet_count: parseInt(count), packet_size: parseInt(size), entropy: parseFloat(entropy) })
        });
        
        const data = await response.json();
        
        if(data.status.includes('MALICIOUS')) {
            document.querySelector('.modal-content').style.borderTopColor = 'var(--red)';
            modalStatus.style.color = 'var(--red)';
            modalStatus.innerText = '⚠️ ' + data.status + ' ⚠️';
        } else {
            document.querySelector('.modal-content').style.borderTopColor = 'var(--cyan)';
        ...
        modalReason.innerHTML = '<div class="exp-block exp-why">Failed to connect to backend AI Server.</div>';
    }NDSS_App/frontend/app.js
});
